"""create_view_voc_15_nvsratingtime

Revision ID: 0f43ce2818fd
Revises: 4dde53d8a362
Create Date: 2021-06-25 03:36:49.521327
...
revision = '0f43ce2818fd'
down_revision = '4dde53d8a362'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    query_job .result()
    print("\n Create view VOC_15_nvsratingtime successfull!")
    
def downgrade():
    query_job =bqclient.query(query_string2)
    query_job .result()
    print("\n Downgrade 4dde53d8a362 process is successfull!")

query_string1 = f"""
CREATE OR REPLACE VIEW `{project}.{datamart}.VOC_15_nvsratingtime` AS 
...
, data as(
    ...
    count(CASE WHEN polarity in ('N','N+','NEU','P','P+') THEN review_id  ELSE null END) as collected_review_count,
    sum( CASE WHEN polarity  in ('N','N+','NEU','P','P+') THEN modified_polarity  ELSE null END) as sum_modified_polarity
...
query_string2 = f"""
DROP VIEW  `{project}.{datamart}.VOC_15_nvsratingtime`;
"""
