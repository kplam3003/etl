"""update_view_name_voc_12_1

Revision ID: abc963bd88c6
Revises: 0c06f2c01f04
Create Date: 2021-08-30 03:30:02.227717
...
revision = 'abc963bd88c6'
down_revision = '0c06f2c01f04'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    query_job .result()
    print("\n Drop view name VOC_12_1_sscasualdim successfull!")
    query_job =bqclient.query(query_string2)
    query_job .result()
    print("\n Update view name VOC_12_1_sscausaldim successfull!")
	
    
def downgrade():
    query_job =bqclient.query(query_string3)
    query_job .result()
    print("\n Drop view name VOC_12_1_sscausaldim successfull!")
    query_job =bqclient.query(query_string4)
    query_job .result()
    print("\n Downgrade 0c06f2c01f04 process is successfull!")


query_string1 = f"""
DROP VIEW  `{project}.{datamart}.VOC_12_1_sscasualdim`;
"""

query_string2 = f"""
CREATE OR REPLACE VIEW `{project}.{datamart}.VOC_12_1_sscausaldim` AS 
...
(
    ...
    WHERE
        dimension_config_name is not null 
        AND modified_dimension is not null 
    GROUP BY
        case_study_id,
        dimension_config_id,
        nlp_type,
...
query_string3 = f"""
DROP VIEW  `{project}.{datamart}.VOC_12_1_sscausaldim`;
"""
query_string4 = f"""
CREATE OR REPLACE VIEW `{project}.{datamart}.VOC_12_1_sscasualdim` AS 
WITH date_case_study AS (
    SELECT
        case_study_id,
        MAX(case_study_name) case_study_name,
