"""fix sum_ss zero to null of voc_5

Revision ID: 65a82cbdc0e1
Revises: c905a09cb298
Create Date: 2021-02-01 13:20:33.560931
...
revision = '65a82cbdc0e1'
down_revision = 'c905a09cb298'
branch_labels = None
depends_on = None
...
def upgrade():
    """fix sum_ss zero to null of voc_5"""
    print("\n running 65a82cbdc0e1!")
    query_job =bqclient.query(query_string1)
    query_job .result()
    print("\n Upgrade 65a82cbdc0e1 process is successfull!")
    
def downgrade():
    query_job =bqclient.query(query_string2)
    query_job .result()
    print("\n Downgrade 65a82cbdc0e1 process is successfull!")
    
query_string1 = f"""

CREATE
OR REPLACE VIEW `{project}.{datamart}.VOC_5_heatmapdim` AS WITH tmp AS (
    SELECT
        case_study_id,
        case_study_name,
...
count_dimension AS (
    SELECT
        ...
        sum(
            CASE
                WHEN modified_polarity is null THEN 0
                ELSE modified_polarity
            END
        ) as sum_ss,
        count(
    ...
    FROM
        `{project}.{datamart}.summary_table`
    WHERE
        dimension is not null # and case_study_id =  144
    GROUP BY
        case_study_id,
        dimension_config_id,
        nlp_type,
